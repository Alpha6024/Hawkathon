...
export default function SymptomChecker() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      // Auto-fill age if patient is logged in
      if (patientData.age) {
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="bg-slate-800 rounded-2xl p-6 border border-slate-700 mb-6 transform transition-all duration-700 ease-out" style={{ transform: isVisible ? 'translateY(0)' : 'translateY(30px)', opacity: isVisible ? 1 : 0 }}>
          ...
          <button
            onClick={handleCheck}
            disabled={loading}
            className={`w-full py-4 rounded-2xl font-bold text-base transition-all duration-300 flex items-center justify-center gap-3 ${loading ? "bg-purple-800 cursor-not-allowed" : "bg-purple-600 hover:bg-purple-500 hover:scale-[1.02] shadow-lg shadow-purple-900/40 hover:shadow-xl"} text-white`}
          >
            {loading ? (
              <>
          ...
          <div className="space-y-4 animate-fadeIn">
            ...
            <div className="bg-blue-900/20 border border-blue-800/40 rounded-2xl p-5 flex items-center justify-between flex-wrap gap-4">
              ...
              <button
                ...
                className="bg-emerald-600 hover:bg-emerald-500 hover:scale-[1.02] text-white px-6 py-3 rounded-xl font-bold text-sm transition-all duration-300 shadow-lg shadow-emerald-900/40 hover:shadow-xl"
              >
